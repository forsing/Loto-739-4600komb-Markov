...
def _full_row_for_history(history: np.ndarray) -> Row:
    ...
    if succ_pair:
        ...
        rule = "pravilo 2: vertikalni lanac (Num_k | Num_1…Num_{k-1}, bez Markov/kNN)"

    last_t = tuple(int(x) for x in history[-1])
    if out == last_t:
        out = _predict_vertical_conditional_not_equal_last(history)
        rule = "pravilo 2b: vertikalni lanac — izbegnut predlog identičan poslednjem poznatom redu"
...
def main(argv: List[str]) -> int:
    ...
    print("Predlog sledećeg izvlačenja (naredni red u nizu, posle zadnjeg iz CSV):", pred)
    if pred == last_known:
